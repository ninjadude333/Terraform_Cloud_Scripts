...
def run_api_get_projects(org_name,token,search_string):
    print(search_string)
    if not search_string:
        url = "https://app.terraform.io/api/v2/organizations/" + org_name + "/projects"
    else:
        url = "https://app.terraform.io/api/v2/organizations/" + org_name + "/projects?q=" + search_string
    print(url)
    ...
    for x in range(0,len(jsonResponse["data"])):
        if verbose:
            print("PRJ Name: {0} *** PRJ ID: {1}".format(jsonResponse["data"][x]["attributes"]["name"],jsonResponse["data"][x]["id"]))
        ids.append(jsonResponse["data"][x]["id"])
    return ids


parser = argparse.ArgumentParser(
    prog='Fetch_TFC_Cloud_Projectss.exe',
    formatter_class=argparse.RawDescriptionHelpFormatter,
    description=('''\
        This tool will get all projects names and id's from Terraform Cloud
        Ver 1.0.0
        Coded By The_Dude
        '''),
    epilog=('''\
        ...
        Fetch_TFC_Cloud_Projects.exe -org MyOrg -token <myToken>
        
        -search_string can be used to add a filter word for the Project query, this word will be wrapped with wilcards.  *my_filter_string*
...
if __name__ == "__main__":
    id_list = run_api_get_projects(org_name,token,s_string)
